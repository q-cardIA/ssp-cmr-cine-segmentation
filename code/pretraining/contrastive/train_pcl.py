"""
A script that applies PCL pretraining on a U-Net model from the qcardia-models package
...
def main():
    ...
    # setup positional contrastive loss labels
    num_key = "slice_nr"
    den_key = "total_nr_slices"
    ...
    for epoch_nr in range(max_epochs):
        ...
        for x in train_dataloader:
            ...
            with torch.autocast(device.type, torch.float16, enabled=use_amp):
                ...
                # get labels to calculate contrastive loss with
                labels = x["meta_dict"][num_key] / x["meta_dict"][den_key]
